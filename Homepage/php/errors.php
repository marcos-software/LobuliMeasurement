...
<?php
    ...
    readfile("../html/head.html");	
		...
		$statement = $pdoLog->prepare("select * from vCut where bExcludeFromAnalysis = 0 and (kCut in (select distinct kCut from vResult where cValue = '0' or cValue = '0,00' or (cValue + 0.0 < 0)) OR kCut not in (select distinct kCut from vCut))");
		$statement->execute();
