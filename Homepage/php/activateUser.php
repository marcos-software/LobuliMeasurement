...
<?php
	...
	if(is_null($mail) == false && isset($mail) && trim($mail) !== '')	
	{
		require_once('db.php');
		...
		$statement              = $pdoLog->prepare("UPDATE tUsers SET bActive = IF(tUsers.bActive = 1, 0, 1) WHERE tUsers.cMail = :cMail");
		$statement->execute(array('cMail' => $mail));
		$statement->fetch();		
	} 
